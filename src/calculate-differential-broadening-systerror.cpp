...
int main(int argc, char* argv[])
{
    ...
    // Create output file
    TFile* fout = new TFile("../output-files/results-differential-broadening-systerr.root","RECREATE");
    gROOT->cd();
    ...
    // Nominal histo
    TH1F* h_broadening_nominal[N_Q2][N_Nu][N_broadening];
    // Systematic histos
    TH1F* h_broadening_syst[syst_index_size][syst_variations][N_Q2][N_Nu][N_broadening];
    // Deviation histos
    TH1F* h_broadening_devs[syst_index_size][syst_variations][N_Q2][N_Nu][N_broadening];
    // Syst Error histos
    TH1F* h_broadening_systerr[syst_index_size][N_Q2][N_Nu][N_broadening];
    ...
    for(int Q2_bin = 0 ; Q2_bin < N_Q2 ; Q2_bin++)
    {
        for(int Nu_bin = 0 ; Nu_bin < N_Nu ; Nu_bin++)
        {
            ...
            {
                h_broadening_nominal[Q2_bin][Nu_bin][targ_index] = (TH1F*)fin->Get(("broadening_Zh_"+broadening_targets[targ_index]+"_"+std::to_string(Q2_bin)+std::to_string(Nu_bin)).c_str());
            }
        }
    }
