...
int main(int argc, char* argv[])
{
    ...
    for(int systsource_index = 0 ; systsource_index < syst_index_size ; systsource_index++)
    {
        for(int targ_index = 0 ; targ_index < N_broadening ; targ_index++)
        {
            ...
            {
                std::string syst_plot_name = (syst[syst_index_vector[systsource_index]]=="rad") ? "accrc_broadening_"+broadening_targets[targ_index] :
                                                                                                  "broadening_"+broadening_targets[targ_index];
                        
                h_broadening_syst[systsource_index][systvar_index][targ_index] = (TH1F*)fin_syst[systsource_index][systvar_index]->Get((syst_plot_name).c_str());
    ...
    for(int targ_index = 0 ; targ_index < N_broadening ; targ_index++)
    {
        for(int systsource_index = 0 ; systsource_index < syst_index_size ; systsource_index++)
        {
            for(int systvar_index = 0 ; systvar_index < syst_variations ; systvar_index++)
            {
                ...
                // Store
                fout->cd();
                h_broadening_devs[systsource_index][systvar_index][targ_index]->Write(("dev_broadening_"+syst[syst_index_vector[systsource_index]]+"_"+std::to_string(systvar_index)+"_"+broadening_targets[targ_index]).c_str());
                gROOT->cd();
            }
            ...
            // Store
            fout->cd();
            h_broadening_systerr[systsource_index][targ_index]->Write(("systerr_broadening_"+syst[syst_index_vector[systsource_index]]+"_"+broadening_targets[targ_index]).c_str());
            gROOT->cd();
        }
    }
