...
int main(int argc, char* argv[])
{
    ...
    // Create output file
    TFile* fout = new TFile("../output-files/results-broadening-systerr.root","RECREATE");
    gROOT->cd();
    ...
    for(int index = 0 ; index < syst_index_size ; index++)
    {
        ...
        std::string file_name_a = input_dir+broadening_file_template+syst[syst_index[index]]+"-a"+extension;
        std::string file_name_b = input_dir+broadening_file_template+syst[syst_index[index]]+"-b"+extension;
